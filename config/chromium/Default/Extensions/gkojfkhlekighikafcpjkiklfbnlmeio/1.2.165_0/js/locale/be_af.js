'use strict'; /*jshint -W100, browser:true, es5:true*/
define(function(){
var E = {
    "GL": {message: "Groenland"},
    "DJ": {message: "Djiboeti"},
    "JM": {message: "Jamaika"},
    "PG": {message: "Papoea Nieu-Guinee"},
    "AT": {message: "Oostenryk"},
    "Want Hola on other devices? (Xbox, PS, Apple TV, iPhone...). Click here": {message: "Wil Hola op ander toestelle? (Xbox, PS, Apple TV, iPhone ...). Klik hier"},
    "SZ": {message: "Swaziland"},
    "BN": {message: "Broenei"},
    "ZM": {message: "Zambië"},
    "app_desc": {message: "Toegang tot al die Internet! Gaan na geblokkeer webwerf -> kliek Hola ikoon -> verandering land vlag -> Geniet dit!"},
    "BW": {message: "Botswana"},
    "AO": {message: "Angola"},
    "Improve translation": {message: "Verbeter vertaling"},
    "ZW": {message: "Zimbabwe"},
    "VC": {message: "Saint Vincent en die Grenadine"},
    "more...": {message: "meer ..."},
    "PR": {message: "Puerto Rico"},
    "JP": {message: "Japan"},
    "NA": {message: "Namibië"},
    "TJ": {message: "Tadjikistan"},
    "There seems to be an error": {message: "Daar blyk 'n probleem te wees"},
    "MA": {message: "Marokko"},
    "SV": {message: "Salvador"},
    "MT": {message: "Malta"},
    "MN": {message: "Mongolië"},
    "IT": {message: "Italië"},
    "RE": {message: "Réunion"},
    "WS": {message: "Samoa"},
    "Unblocker": {message: "Ontstopper"},
    "Enable": {message: "In staat stel om"},
    "Access any site from any country, free": {message: "Toegang tot enige perseel van enige land, gratis"},
    "Loading": {message: "Laai"},
    "Try to <span>reload</span>": {message: "Probeer om <span> herlaai </span>"},
    "FR": {message: "Frankryk"},
    "EG": {message: "Egipte"},
    "UZ": {message: "Oesbekistan"},
    "start": {message: "begin"},
    "LR": {message: "Liberië"},
    "RW": {message: "Rwanda"},
    "BE": {message: "België"},
    "TN": {message: "Tunisië"},
    "EE": {message: "Estland"},
    "BY": {message: "Wit-Rusland"},
    "Accelerator": {message: "Accelerator"},
    "KR": {message: "Suid-Korea"},
    "SA": {message: "Saoedi-Arabië"},
    "NO": {message: "Noorweë"},
    "LS": {message: "Lesotho"},
    "ZA": {message: "Suid-Afrika"},
    "PT": {message: "Portugal"},
    "BF": {message: "Boerkina Fasso"},
    "CA": {message: "Kanada"},
    "AM": {message: "Armenië"},
    "Starting...": {message: "Begin ..."},
    "CM": {message: "Kameroen"},
    "SR": {message: "Suriname"},
    "MG": {message: "Madagaskar"},
    "The free version of Hola needs to be re-enabled every 4 hours and is sponsored by ads.": {message: "Die gratis weergawe van Hola moet weer in staat gestel om elke 4 uur en word geborg deur advertensies."},
    "ON": {message: "OP"},
    "Hola": {message: "Hola"},
    "BT": {message: "Bhoetan"},
    "PL": {message: "Pole"},
    "TM": {message: "Turkmenië"},
    "CF": {message: "Sentraal-Afrikaanse Republiek"},
    "GA": {message: "Gaboen"},
    "AE": {message: "Verenigde Arabiese Emirate"},
    "BA": {message: "Bosnië en Herzegowina"},
    "TH": {message: "Thailand"},
    "KY": {message: "Kaaimanseilande"},
    "Author:": {message: "Skrywer:"},
    "Verify you have Internet": {message: "Verifieer jy Internet"},
    "LA": {message: "Laos"},
    "PH": {message: "Filippyne"},
    "NI": {message: "Nicaragua"},
    "NC": {message: "Nieu-Kaledonië"},
    "KZ": {message: "Kasakstan"},
    "NR": {message: "Naoeroe"},
    "NE": {message: "Niger"},
    "AD": {message: "Andorra"},
    "MR": {message: "Mouritanië"},
    "TO": {message: "Tonga"},
    "SE": {message: "Swede"},
    "changing...": {message: "verander ..."},
    "AZ": {message: "Aserbeidjan"},
    "AF": {message: "Afganistan"},
    "NG": {message: "Nigerië"},
    "KE": {message: "Kenia"},
    "BJ": {message: "Benin"},
    "Turn on to get started": {message: "Draai op om te begin"},
    "Very old version of Chrome, <a>update</a> Chrome to use Hola": {message: "Baie ou weergawe van Chrome <a> by </a> Chrome Hola te gebruik"},
    "OM": {message: "Oman"},
    "VN": {message: "Viëtnam"},
    "YE": {message: "Jemen"},
    "DZ": {message: "Algerië"},
    "CI": {message: "Ivoorkus"},
    "LK": {message: "Sri Lanka"},
    "ID": {message: "Indonesië"},
    "FM": {message: "Mikronesië"},
    "(some Hola features are not available on your version)": {message: "(Sommige Hola funksies is nie beskikbaar op jou weergawe)"},
    "GE": {message: "Georgië"},
    "GM": {message: "Gambië"},
    "LV": {message: "Letland"},
    "RU": {message: "Rusland"},
    "LB": {message: "Libanon"},
    "FK": {message: "Falklandeilande"},
    "DE": {message: "Duitsland"},
    "FI": {message: "Finland"},
    "MV": {message: "Maldive"},
    "Get Hola Plus for un-interrupted, ad-free service.": {message: "Kry Hola Plus vir un-onderbreek word, advertensie-gratis diens."},
    "even more...": {message: "selfs meer ..."},
    "LU": {message: "Luxemburg"},
    "VE": {message: "Venezuela"},
    "BH": {message: "Bahrein"},
    "RO": {message: "Roemenië"},
    "AR": {message: "Argentinië"},
    "IN": {message: "Indië"},
    "CS": {message: "Serwië en Montenegro"},
    "SN": {message: "Senegal"},
    "MX": {message: "Meksiko"},
    "FO": {message: "Faroëreilande"},
    "Unblocker site list": {message: "Ontstopper webwerf lys"},
    "MC": {message: "Monaco"},
    "BR": {message: "Brasilië"},
    "IL": {message: "Israel"},
    "SB": {message: "Solomon Eilande"},
    "Author site:": {message: "Skrywer webwerf:"},
    "Translate to your language": {message: "Vertaal na jou taal"},
    "NZ": {message: "Nieu-Seeland"},
    "DO": {message: "Dominikaanse Republiek"},
    "HU": {message: "Hongarye"},
    "UG": {message: "Uganda"},
    "OFF": {message: "AF"},
    "KH": {message: "Kambodja"},
    "GB": {message: "Groot-Brittanje"},
    "Hola cannot work properly because another extension is controlling your proxy settings. Please disable other extensions that you think might control your proxy settings in <a>extensions</a> (such as ad-blockers, other VPN services, etc.).": {message: "Hola kan nie behoorlik werk nie, want 'n ander uitbreiding is die beheer van jou proxy-instellings. Skakel ander uitbreidings wat jy dink dat jou instaanbedieneropstelling in <a> Uitbreidings </a> (soos die advertensie-blokkers, ander Skynprivaatnetwerk dienste, ens) kan beheer."},
    "Remove ads": {message: "Verwyder advertensies"},
    "HT": {message: "Haïti"},
    "DK": {message: "Denemarke"},
    "PA": {message: "Panama"},
    "QA": {message: "Katar"},
    "CV": {message: "Kaap Verde"},
    "Reload": {message: "Herlaai"},
    "Number of users that use this option": {message: "Aantal gebruikers is wat hierdie opsie"},
    "MO": {message: "Macao"},
    "GF": {message: "Frans-Guyana"},
    "Old version of Firefox. Press <a>here</a> to upgrade.": {message: "Ou weergawe van Firefox. Press <a> hier </a> op te gradeer."},
    "KM": {message: "Comore"},
    "HR": {message: "Kroasië"},
    "KW": {message: "Koeweit"},
    "CZ": {message: "Tjeggiese Republiek"},
    "MZ": {message: "Mosambiek"},
    "ES": {message: "Spanje"},
    "BO": {message: "Bolivië"},
    "AU": {message: "Australië"},
    "ST": {message: "Sao Tome en Principe"},
    "AL": {message: "Albanië"},
    "IR": {message: "Iran"},
    "CG": {message: "Kongo"},
    "TR": {message: "Turkye"},
    "GW": {message: "Guinee-Bissau"},
    "GN": {message: "Guinee"},
    "MK": {message: "Macedonië"},
    "GR": {message: "Griekeland"},
    "AG": {message: "Antigua en Barbuda"},
    "SI": {message: "Slowenië"},
    "Settings": {message: "Instellings"},
    "AQ": {message: "Antarktika"},
    "AN": {message: "Nederlands-Antille"},
    "JO": {message: "Jordanië"},
    "SM": {message: "San Marino"},
    "UA": {message: "Oekraine"},
    "EH": {message: "Wes-Sahara"},
    "CL": {message: "Chili"},
    "CU": {message: "Kuba"},
    "KN": {message: "Saint Kitts en Nevis"},
    "ML": {message: "Mali"},
    "SC": {message: "Seychelle"},
    "ET": {message: "Ethiopië"},
    "IS": {message: "Ysland"},
    "NL": {message: "Nederland"},
    "Reload Hola": {message: "Herlaai Hola"},
    "HK": {message: "Hongkong"},
    "MY": {message: "Maleisië"},
    "CR": {message: "Costa Rica"},
    "VA": {message: "Vatikaan"},
    "back to": {message: "terug na"},
    "SD": {message: "Soedan"},
    "CN": {message: "Sjina"},
    "BG": {message: "Bulgarye"},
    "MH": {message: "Marshall-eilande"},
    "BS": {message: "Bahamas"},
    "PY": {message: "Paraguay"},
    "MU": {message: "Mauritius"},
    "CH": {message: "Switserland"},
    "LI": {message: "Liechtenstein"},
    "GH": {message: "Ghana"},
    "KG": {message: "Kirgisië"},
    "US": {message: "Verenigde State van Amerika"},
    "PE": {message: "Peru"},
    "SL": {message: "Sierra Leone"},
    "CY": {message: "Ciprus"},
    "FJ": {message: "Fidji"},
    "IE": {message: "Ierland"},
    "Get help from engineer over skype:": {message: "Kry hulp van ingenieur oor Skype:"},
    "TW": {message: "Taiwan"},
    "KP": {message: "Noord-Korea"},
    "app_name": {message: "Hola Beter Internet"},
    "IQ": {message: "Irak"},
    "AS": {message: "Amerikaans Samoa"},
    "Update": {message: "Werk"},
    "TZ": {message: "Tanzanië"},
    "LY": {message: "Libië"},
    "Unblocker is disabled": {message: "Ontstopper is gedeaktiveer"},
    "$1 blocked?": {message: "$1 geblokkeer?"},
    "GQ": {message: "Ekwatoriaal-Guinee"},
    "PK": {message: "Pakistan"},
    "$1 unblocked": {message: "$1 geblokkeer"},
    "LT": {message: "Litaue"},
    "Number of users that pressed not working": {message: "Aantal gebruikers wat gedruk nie werk nie"},
    "SG": {message: "Singapoer"},
    "Initializing...": {message: "Inisialiseer ..."},
    "TT": {message: "Trinidad en Tobago"},
    "Hola does not work well in Windows 8 mode. Please switch to desktop mode. Click <a>here</a> for instructions": {message: "Hola nie goed werk nie in Windows 8 af. Skakel asseblief aan lessenaar af. Klik <a> hier </a> vir instruksies"},
    "TD": {message: "Tsjaad"},
    "SO": {message: "Somalië"},
    "SK": {message: "Slowakye"},
    "SY": {message: "Sirië"},
    "BD": {message: "Bangladesj"},

};
return E; });