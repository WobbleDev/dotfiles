...
define(['jquery', 'underscore', 'backbone', 'spin', 'be_popup_lib', 'be_util',
    'be_locale', 'bootstrap', 'be_popup_main', 'be_browser', 'be_zerr'],
    function($, _, Backbone, spin, be_popup_lib, be_util, T, bootstrap,
    be_popup_main, B, zerr){
B.assert_popup();
...
function $err_msg(_class){
    /* XXX arik hack:  temporary change all plugin to ext on error */
    if (0 && be_util.is_plugin())
    {
        be_util.storage_set('set_tmp_not_plugin', 1);
	setTimeout(function(){
	    be_popup_lib.reload_ext({info: 'plugin_auto_recover'}); });
    }
	...
	.click(function(e){ be_popup_lib.reload_ext({info: 'error'}); })));
    $list.append($('<li>').text(T('Verify you have Internet')));
...
E.changing_view = Backbone.View.extend({
	...
	if (src && dst)
	{
	    $('<div>', {class: 'r_ui_in_progress f32'}).appendTo($el)
	    .append($('<span>', {class: 'flag '+src.toLowerCase()}),
		$('<span>').text(' '+T(src)),
		$('<span>', {class: 'arrow'}).html(' &rarr; '),
...
E.tooltip = Backbone.View.extend({
	...
	$parent.mouseenter(function(){ this.show(); }.bind(this))
...
E.select = Backbone.View.extend({
	...
	if (this.options.pull_right)
        ...
        if (!chrome) /* XXX arik hack: force resize of popup */
	    $el.click(function(){ be_popup_main.resize(100); });
    },
...
E.lang_list = Backbone.View.extend({
	...
	if (T.locales.length==1)
	    return;
	var select = new E.select({local_css: true});
	select.$el.appendTo($el);
	for (var i=0; i<T.locales.length; i++)
	{
	    var l = T.locales[i];
	    select.add_item($('<span>', {text: T('locale_en_'+l)}), l);
	}
	/* XXX vladimir: check if .capital and _locale='en' are required */
	select.add_item($('<span>', {text: T.capital('more...', null, 'en')}),
	    'more');
	select.val(T.locale);
	...
	select.$btn.addClass('l_btn-rm-border');
	...
	var tt = new E.tooltip({$parent: select.$el, $content: $a});
	select.$el.click(function(){ tt.hide(); });
    },
});
