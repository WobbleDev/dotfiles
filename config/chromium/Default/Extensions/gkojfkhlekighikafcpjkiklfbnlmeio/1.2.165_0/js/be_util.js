...
E.json_opt = function(opt){
	...
	var t = Date.now()-t0;
	zerr._zerr(zerr.L[t>slow ? 'ERR' : 'DEBUG'],
	...
	return etask.p.reject(new Error(errorThrown));
    }]);
};
...
E.ajax_opt = function(opt){
	...
	if ((isNaN(retval) || retval))
	{
	    ...
	    return etask.p.reject(new Error('ajax onfail'));
	}
	var t = Date.now()-t0;
	zerr._zerr(zerr.L[t>slow ? 'ERR' : 'DEBUG'],
    ...
    return etask([function(){
        ...
        chrome.windows.getAll({}, function(wins){
            ...
            for (var i=0; i<l; i++)
            {
                ...
                if (wins[i].width!==window.screen.width ||
                    wins[i].height!==window.screen.height)
                {
...
E.svc_callback = function(timeout){
    var old = false;
    var opt = {dataType: 'json', cache: false, timeout: timeout||5000};
    return etask([function(){
	$.ajax($.extend({}, opt, {url: conf.ipc+'callback.json',
            error: function(xhr, status){
                this.econtinue([xhr, status]); }.bind(this),
	    success: function(data, status){
                this.egoto('success', [data, status]); }.bind(this)}));
	return this.wait();
    }, function(r){
        var xhr = r[0];
        if (xhr.status!=404)
        ...
        $.ajax($.extend({}, opt, {url: conf.ipc+'callback.cgi?cid',
            success: function(data, status){
                this.econtinue([data, status]); }.bind(this),
            error: function(){ /* XXX bahaa/yoni: throw? */
                this.egoto('throw_err'); }.bind(this)}));
	return this.wait();
    }, function success(r){
	...
	if (!E.is_plugin() && !is_svc)
        ...
        /* XXX bahaa/yoni: can we remove this state and throw while in wait? */
        throw new Error('not running');
    }]);
};
...
E.init = function(){
    if (E.storage_get_int('use_http', 1))
