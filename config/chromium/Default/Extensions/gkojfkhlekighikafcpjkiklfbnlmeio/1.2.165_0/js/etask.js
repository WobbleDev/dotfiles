...
E.prototype._run = function(){
	...
	if (E.p.is_promise(ret))
	{
		...
		ret.then(function(ret){ run_after_promise(_this, ret);
		    }, function(err){ run_after_promise(_this, undefined,
		    err); });
		return;
	    }
	}
...
E.prototype.ethrow = function(err){
    return this.ereturn(E.p.reject(err));
};
...
function _ps(et, pre_first, pre_next, show_sp, flags){
    ...
    function trim_cat(cat){ s = s.replace(/\s+$/, "")+cat; }
...
E.ps = function(et, flags){
    var i, s = '', task_trail;
    if (arguments.length==1 && !(et instanceof Etask) &&
...
function nodefn_multi(fn, arg_names, ret_name)
{
    return function(){
        var ret;
...
var v = {'fulfilled': 1, 'rejected': -1};
E.p.get_status = function(promise){
    return is_d(promise) ? promise.getStatus() : v[promise.state]; };
E.p.get_value = function(promise){
    if (is_d(promise))
    {
	var v = promise.valueOf();
	return v===promise ? undefined : v;
    }
...
E.all = function(a_or_o){
	...
	return E.p._all(a_or_o);
    if (!(a_or_o instanceof Object))
	throw 'invalid type';
    var names = [], promises = [], i;
    for (i in a_or_o)
    {
	names.push(i);
	promises.push(a_or_o[i]);
    }
    return E.all(promises)
    .then(function(res){
	var o = {};
...
E._nodefn_call = function(func, _this /*, args */){
    return E.p._nodefn_apply(func, _this, array.slice(arguments, 2)); };
/* same like when.nodefn but faster, since we dont resolve every single
 * argument */
E.nodefn_call = function(func /*, args */){
    return E.p._nodefn_apply(func, null, array.slice(arguments, 1)); };
