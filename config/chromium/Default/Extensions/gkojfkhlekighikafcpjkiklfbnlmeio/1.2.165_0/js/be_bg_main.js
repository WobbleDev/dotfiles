...
E.on_ccgi_url_change = function(){
    var svc = E.get('svc.running'), plugin = be_util.is_plugin(), base;
    var qs = {browser: E.get('browser'), ver: be_util.version(),
        plugin: +(plugin && !svc)};
    if (svc || plugin)
    {
        base = E.get('ccgi_url_base')||conf.url_ccgi+'/';
        if (E.get('cid'))
            qs.cid = E.get('cid');
    }
    else
    {
        base = conf.url_be_ccgi+'/';
        if (E.get('uuid'))
            qs.uuid = E.get('uuid');
    }
    E.set('ccgi_url', base+'?'+$.param(qs));
};
...
E.init = function(install_details){
    ...
    E.on_init('change:browser change:cid change:ccgi_url_base change:uuid '+
        'change:svc.running', E.on_ccgi_url_change);
